...
set "foldername=Dataiku_Model_API"
...
REM And we also exclude .git, create_archive.bat, download_packages.bat, setup.bat and venv files
set "psfile=%temp%\create_archive_temp.ps1"
> "%psfile%" echo $src = (Get-Location).Path
>> "%psfile%" echo $dest = Join-Path (Join-Path $src '..') '%zipname%'
>> "%psfile%" echo $excludes = @('.git', '.gitignore', 'create_archive.bat', 'download_packages.bat', 'setup.bat', 'venv')
